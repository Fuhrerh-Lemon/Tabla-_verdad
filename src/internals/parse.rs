...
impl Parsing {
    // solo verifica la igualdad para dos tipos de Parsing :: String, de lo contrario es falso
...
fn downgrade<R>(range: R, parsing: &mut Vec<Parsing>) where R: RangeBounds<usize> {
    let index = match range.start_bound() {
        Bound::Included(t) => *t,
        Bound::Unbounded => 0,
        _ => unreachable!("El índice de inicio no debe estar BOUND::excluido")
    };
                ...
                3 => {
                    if let Parsing::String(s) = &l[1] {
                        ...
                        match s.as_str() {
                            ...
                            _ => panic!("inesperado '{}'", s)
                        }
                    } else {
                        unreachable!("Análisis no válido :: SubList en medio de un bloque de 3 largos")
                    }
                },
                n => unreachable!("invalida longitud de bloques: {}", n)
            }
        }
    }
}
